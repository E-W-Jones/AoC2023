# Someone elses day 20 solution: https://aoc-puzzle-solver.streamlit.app/
# Used it for debugging mine
# coding: utf-8
from collections import deque
from itertools import count
import math

def part2(puzzle_input, verbose=False):
    graph = {}
    flip_flop = {}
    memory = {}
    ...
    if verbose: print(f"{semi_final_layer = }")    
    ...
    for button_push in count(1):
        ...
        while queue:
            ...
            if verbose: print(f"{out_module} -{'high' if signal else 'low'}-> {in_module}")
        ...
        if not semi_final_layer:
            break
    if verbose: print(cycle_lengths)
    return math.lcm(*cycle_lengths)

with open("input.txt", "r") as filein:
    print(part2(filein.read(), verbose=True))